...
function buildDealReceiptLine(dealName, groupQty, instance, savingsPerInstance = 0) {
  const componentTotal = instance.reduce(
    (sum, c) => sum + Number(c.unitPrice ?? c.price ?? 0) * itemQty(c),
    0,
  );
  const perInstanceComponent =
    instance.reduce((sum, c) => sum + Number(c.unitPrice ?? c.price ?? 0), 0);
  const unitPrice = Math.max(
    0,
    Math.round((perInstanceComponent - savingsPerInstance) * 100) / 100,
  );
  ...
  return {
    ...
    _componentTotal: componentTotal,
  };
}
...
function collapseAppliedDealsForReceipt(items, appliedDeals = []) {
  ...
  for (const applied of comboDeals) {
    ...
    const savingsPerInstance =
      instanceCount > 0
        ? Math.round((totalSavings / instanceCount) * 100) / 100
        : 0;
    ...
    for (const { instance, count: groupQty } of groups.values()) {
      lines.push(
        buildDealReceiptLine(dealName, groupQty, instance, savingsPerInstance),
      );
    }
  }
