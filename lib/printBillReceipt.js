...
import { formatReceiptItemsForBill, formatOrderItemDisplayName, getOrderItemBillModifierLines } from "./orderDisplay.js";
    ...
    .map((it, index) => {
      ...
      const dealChoicesHtml = it.isDealLine
        ? (it.dealChoices || [])
            .map(
              (choice) =>
                `<tr><td colspan="3" style="padding:0 0 2px;border-bottom:0;padding-left:14px;font-size:11px;color:#444;">· ${escapeHtml(choice.name)} x${choice.qty}</td></tr>`,
            )
            .join("")
        : "";
        ...
        it.isDealLine
          ...
          : getOrderItemBillModifierLines(it)
              .map((opt) => {
                ...
                return `<tr>
            <td colspan="2" style="padding:0 0 2px;border-bottom:0;padding-left:16px;font-size:11px;color:#666;font-style:italic;">
              + ${escapeHtml(opt.name)}
            </td>
            <td style="padding:0 0 2px;border-bottom:0;font-size:11px;color:#666;text-align:right;">
              ${priceSuffix}
            </td>
          </tr>`;
              })
      ...
      const dealBadge = it.isDealLine
        ? `<div style="font-size:10px;color:#777;margin-top:1px;">Combo deal</div>`
        : "";
