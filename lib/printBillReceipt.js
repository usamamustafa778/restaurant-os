...
export function buildBillHtml(orderLike, options = {}) {
  ...
  const normalizedOrderType = String(orderLike.type || "dine-in").toLowerCase();
  const shouldShowTableNo = normalizedOrderType === "dine-in";

  const subtotal = orderLike.subtotal ?? orderLike.total ?? 0;
  const total = orderLike.total ?? 0;

  const customerPhoneBlock = !isWalkInCustomer && customerPhoneRaw
  ...
  const customerAddressBlock =
    !isWalkInCustomer && customerAddressRaw && !isWalkInAddress
    ? `<div style="display:flex;justify-content:space-between;">
...
<body>
  ...
  <div class="center" style="font-size:10px;color:#888;margin-top:16px;border-top:1px dashed #ccc;padding-top:8px;">
    Powered by Eats Desk<br/>eatsdesk.com
  </div>
</body>
</html>`;
}
...
function buildPrintPayload(orderLike, options = {}) {
  ...
  const deliveryAddress =
    isWalkInCustomer || isWalkInAddress ? "-" : rawDeliveryAddress || "-";
