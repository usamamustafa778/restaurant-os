...
import { mergeReceiptItems } from "./orderDisplay.js";
...
export function buildBillHtml(orderLike, options = {}) {
  ...
  const items = mergeReceiptItems(orderLike.items || []);
  ...
  const itemsHtml = items
    .map((it, index) => {
      ...
      const modifierHtml = (it.modifierSelections || [])
        .filter((sel) => {
          const optPrice = (sel.options || []).reduce(
            (s, o) => s + (o.price || 0),
            0,
          );
          // Only show optional add-ons that genuinely cost extra.
          // Skip required groups (their price IS the item price, already shown via variantLabel)
          // and free add-ons.
          return optPrice > 0 && optPrice !== it.unitPrice;
        })
        .map((sel) => {
      ...
      return `<tr>
          <td style="padding:4px 0;${topBorder}border-bottom:1px dashed #000;white-space:normal;">
            ...
            ${it.variantLabel ? `<div style="font-size:10px;color:#555;margin-top:1px;">(${escapeHtml(it.variantLabel)})</div>` : ""}
            ${eachHint}
          </td>
  ...
  const shouldShowTaxLine = showTaxOnBill === true && taxAmount > 0;
  const grandTotal =
...
<br/>
  ...
  <table>
    ...
    ${discountRowsHtml}
    ${shouldShowTaxLine ? `<tr><td><strong>${escapeHtml(`${taxLabel} (${taxRate}%)`)}</strong></td><td style="text-align:right">${Number(taxAmount).toFixed(2)}</td></tr>` : ""}
