...
import { formatReceiptItemsForBill, formatOrderItemDisplayName, getOrderItemBillModifierLines, getDealDisplayItems } from "./orderDisplay.js";
...
export function buildBillHtml(orderLike, options = {}) {
  ...
  const items = formatReceiptItemsForBill(orderLike);
  ...
  const itemsHtml = items
    .map((it, index) => {
      ...
      const dealChoicesHtml = it.isDealLine
        ? getDealDisplayItems(it)
            .map((choice) => {
              const badge = choice.isChoice
                ? ` <strong>[choice]</strong>`
                : "";
              // Thermal printers crush gray text — keep deal lines pure black.
              return `<tr><td colspan="3" style="padding:1px 0 2px;border-bottom:0;padding-left:12px;font-size:13px;font-weight:600;color:#000;">- ${escapeHtml(choice.name)} x${choice.qty}${badge}</td></tr>`;
            })
            .join("")
        : "";
      ...
      const modifierHtml =
        it.isDealLine
          ? ""
          : getOrderItemBillModifierLines(it)
              .map((opt) => {
                const priceSuffix =
                  opt.price > 0 ? `+${opt.price.toFixed(2)}` : "";
                return `<tr>
            <td colspan="2" style="padding:1px 0 2px;border-bottom:0;padding-left:12px;font-size:13px;font-weight:600;color:#000;">
              + ${escapeHtml(opt.name)}
            </td>
            <td style="padding:1px 0 2px;border-bottom:0;font-size:13px;font-weight:600;color:#000;text-align:right;">
              ${priceSuffix}
            </td>
          </tr>`;
              })
      ...
      const eachHint =
        qty > 1
          ? `<div style="font-size:12px;font-weight:600;padding-top:2px;color:#000;">(@ Rs ${Number(unit).toFixed(2)} each)</div>`
          : "";

      const dealBadge = it.isDealLine
        ? `<div style="font-size:12px;font-weight:700;color:#000;margin-top:1px;">Combo deal</div>`
        : "";

      return `<tr>
          <td style="padding:4px 0;${topBorder}border-bottom:1px dashed #000;white-space:normal;">
            <div style="font-weight:600;">${escapeHtml(formatOrderItemDisplayName(it))}</div>
            ${dealBadge}
            ${eachHint}
          </td>
  ...
  const manualDiscountLine =
    Number(orderLike.posManualDiscountPercent || 0) > 0 && manualDiscountAmt > 0
  ...
  const subtotal = Number(orderLike.subtotal ?? orderLike.total ?? 0) || 0;
  const total = Number(orderLike.total ?? 0) || 0;
  const deliveryCharges = isDeliveryOrder
    ? Number(orderLike.deliveryCharges || 0) || 0
    : 0;
  const reservationCharges = Number(orderLike.reservationCharges || 0) || 0;
  const taxAmount = Number(orderLike.taxAmount || 0) || 0;
  const taxRate = Number(orderLike.taxRate || 0) || 0;
  const taxLabel = String(orderLike.taxLabel || "Tax").trim() || "Tax";
  const shouldShowTaxLine = taxAmount > 0;
  const grandTotal =
    orderLike.grandTotal != null && !Number.isNaN(Number(orderLike.grandTotal))
      ? Number(orderLike.grandTotal)
      : total + taxAmount + deliveryCharges + reservationCharges;
  ...
  const returnAmount =
    orderLike.paymentAmountReturned != null
      ...
      : hasPaymentDetails && orderLike.paymentAmountReceived != null
        ? Math.max(
            0,
            Number(orderLike.paymentAmountReceived) - grandTotal,
          )
        : 0;
  ...
  const _dt = orderLike.createdAt ? new Date(orderLike.createdAt) : new Date();
  const orderDate = _dt.toLocaleDateString("en-PK");
  const orderTime = _dt.toLocaleTimeString("en-PK", {
    hour: "2-digit",
    minute: "2-digit",
    hour12: true,
  });
...
<br/>
  <div style="display:flex;justify-content:space-between;">
    <span style="font-weight:700;font-size:12px;">${escapeHtml(String(orderId))}</span>
    <span style="font-size:11px;color:#555;">${escapeHtml(orderDate)}</span>
  </div>
    <hr/>
  ...
  <table>
    ...
    ${discountRowsHtml}
    ${shouldShowTaxLine ? `<tr><td><strong>${escapeHtml(`${taxLabel} (${taxRate}%):`)}</strong></td><td style="text-align:right">Rs ${Number(taxAmount).toFixed(2)}</td></tr>` : ""}
    ...
    ${Number(orderLike.reservationCharges || 0) > 0 ? `<tr><td><strong>Reservation / Party Fee</strong></td><td style="text-align:right">${Number(orderLike.reservationCharges).toFixed(2)}</td></tr>` : ""}
    <tr class="total-row" style="font-size:16px"><td>Grand Total</td><td style="text-align:right">${Number(grandTotal).toFixed(2)}</td></tr>
  </table>
  <hr/>
...
function buildPrintPayload(orderLike, options = {}) {
  ...
  const returnAmount =
    orderLike.paymentAmountReturned != null
      ...
      : hasPayment
        ? Math.max(
            0,
            Number(orderLike.paymentAmountReceived) -
              (orderLike.grandTotal ??
                (Number(orderLike.total || 0) +
                  Number(orderLike.taxAmount || 0) +
                  (String(orderLike.type || orderLike.orderType || "")
                    .toLowerCase()
                    .includes("delivery")
                    ? Number(orderLike.deliveryCharges || 0)
                    : 0))),
          )
        : 0;
  ...
  const total = Number(orderLike.total ?? 0) || 0;
  const taxAmount = Number(orderLike.taxAmount || 0) || 0;
  const deliveryCharges = isDeliveryOrder ? Number(orderLike.deliveryCharges || 0) || 0 : 0;
  const reservationCharges = Number(orderLike.reservationCharges || 0) || 0;
  const grandTotal =
    orderLike.grandTotal != null && !Number.isNaN(Number(orderLike.grandTotal))
      ? Number(orderLike.grandTotal)
  ...
  const _payloadDt = orderLike.createdAt
    ? new Date(orderLike.createdAt)
    : new Date();
  return {
    ...
    orderDate: _payloadDt.toLocaleDateString("en-PK"),
    createdAt: _payloadDt.toLocaleTimeString("en-PK", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
    }),
    ...
    waiter:
      ...
      (orderLike.createdBy && orderLike.createdBy.name) ||
      "N/A",
    items: formatReceiptItemsForBill(orderLike),
    ...
    taxRate: Number(orderLike.taxRate || 0) || 0,
    taxAmount,
    taxLabel: String(orderLike.taxLabel || "Tax").trim() || "Tax",
    taxableAmount: Number(orderLike.taxableAmount || 0) || 0,
