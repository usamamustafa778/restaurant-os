    ...
    selectionsBySlot[slotIndex] = [
      {
        ...
        name: match.name,
        qty: match.qty ?? match.quantity ?? 1,
...
export function rebuildPosCartFromOrder(order, menuItems = [], availableDeals = []) {
  ...
  const appliedDeals = order?.appliedDeals || [];
  const comboApplied = appliedDeals.filter((d) => d.dealType === "COMBO");
  ...
  for (const applied of comboApplied) {
    const dealId = String(applied.dealId || applied.deal?._id || applied.deal || "");
    if (!dealId) continue;
    const dealName = applied.dealName || "Deal";
    const dealDoc = availableDeals.find((d) => String(d._id || d.id) === dealId);
    ...
    const comboPrice =
      ...
      Math.max(0, componentTotal - (Number(applied.discountAmount) || 0));
