...
function normModifierName(value) {
  return String(value || "").trim().toLowerCase();
}

function findOrderModifierGroup(cartItem, group) {
  const gid = String(group.id);
  const gname = normModifierName(group.groupName || group.name);
  for (const sel of cartItem._modifierSelectionsForOrder || []) {
    if (String(sel.groupId) === gid) return sel;
    if (gname && normModifierName(sel.groupName) === gname) return sel;
  }
  return null;
}

function optionExistsInGroup(group, optionId) {
...
function matchOptionForGroup(group, { optionName, unitPrice, variantHint } = {}) {
  const options = (group.options || []).filter((o) => o.isAvailable !== false);
  if (optionName) {
    const byName = options.find(
      (o) => normModifierName(o.name) === normModifierName(optionName),
    );
    if (byName) return byName;
  }
  if (variantHint) {
    const byVariant = options.find(
      (o) => normModifierName(o.name) === normModifierName(variantHint),
    );
    if (byVariant) return byVariant;
  }
  if (unitPrice != null && Number.isFinite(Number(unitPrice))) {
    const byPrice = options.find((o) => Number(o.price) === Number(unitPrice));
    if (byPrice) return byPrice;
  }
  return null;
}

/** Resolve the menu option id to show as selected for a variation dropdown. */
export function getSelectedOptionIdForGroup(group, cartItem) {
  if (!group || !cartItem) return "";
  const selections = modifierSelectionsFromCartItem(cartItem);
  const current = (selections[String(group.id)] || [])[0];
  if (current?.optionId) {
    const byId = optionExistsInGroup(group, current.optionId);
    if (byId) return String(byId.id);
    if (current.name) {
      const byName = (group.options || []).find(
        (o) =>
          normModifierName(o.name) === normModifierName(current.name) &&
          o.isAvailable !== false,
      );
      if (byName) return String(byName.id);
    }
  }
  const variantHint = String(cartItem.size || cartItem.variantLabel || "").trim();
  if (variantHint) {
    const byVariant = (group.options || []).find(
      (o) =>
        normModifierName(o.name) === normModifierName(variantHint) &&
        o.isAvailable !== false,
    );
    if (byVariant) return String(byVariant.id);
  }
  if (group.required !== false && Number(cartItem.price) > 0) {
    const byPrice = (group.options || []).find(
      (o) =>
        Number(o.price) === Number(cartItem.price) && o.isAvailable !== false,
    );
...
/** Re-match saved order modifier picks to the current branch menu (by name/price). */
export function reconcileOrderModifiersOntoCartItem(cartItem, menuItem) {
  if (!cartItem || !menuItem || cartItem.isDeal || cartItem._isDeal) return cartItem;

  const template = {
    ...menuItem,
    ...cartItem,
    id: menuItem.id || cartItem.id,
    modifierGroups: menuItem.modifierGroups || cartItem.modifierGroups,
    attachedModifierGroups:
      menuItem.attachedModifierGroups || cartItem.attachedModifierGroups,
    hasModifiers: menuItem.hasModifiers ?? cartItem.hasModifiers,
  };

  const groups = getPickerGroupsForItem(template).filter(isSingleSelectModifierGroup);
  if (!groups.length) return cartItem;

  const variantHint = String(cartItem.size || cartItem.variantLabel || "").trim();
  const preservedPrice = Number(cartItem.price) || 0;
  const selections = modifierSelectionsFromCartItem(cartItem);
  let changed = false;

  for (const group of groups) {
    const gid = String(group.id);
    const current = (selections[gid] || [])[0];
    if (current?.optionId && optionExistsInGroup(group, current.optionId)) {
      continue;
    }

    const orderGroup = findOrderModifierGroup(cartItem, group);
    const optionName = orderGroup?.options?.[0]?.name || current?.name;
    const usePriceHint =
      group.required !== false &&
      groups.filter((g) => g.required !== false).length === 1;

    const matched = matchOptionForGroup(group, {
      optionName,
      variantHint: group.required !== false ? variantHint : undefined,
      unitPrice: usePriceHint ? preservedPrice : undefined,
    });
  ...
  if (!changed) return cartItem;

  const line = buildPosCartLine(template, cartItem.quantity || 1, selections);
  return {
    ...cartItem,
    price: preservedPrice || line.unitPrice,
    size: line.variantLabel || cartItem.size,
...
export function buildPosCartItemsFromOrderItems(orderItems = [], menuItems = []) {
  return orderItems.map((it) => {
    ...
    let menuItem = null;
    if (id) {
      menuItem = menuItems.find((m) => String(m.id || m._id) === String(id));
      if (menuItem) imageUrl = menuItem.imageUrl || "";
    } else {
    ...
    return menuItem ? reconcileOrderModifiersOntoCartItem(cartItem, menuItem) : cartItem;
  });
}
...
export function mapPosCartLineToOrderUpdatePayload(item, itemNotes = {}) {
  ...
  return {
    ...
    name: item.name,
    note: itemNotes[cartKey] || item.note || undefined,
    modifierSelections:
    ...
    dealId: item._dealId || undefined,
  };
}

/** Split expanded deal lines into choice picks vs fixed bundle items (website uses "Deal · slot" vs "Deal"). */
function splitDealComponentsByRole(components, dealName) {
  const choice = [];
  const fixed = [];
  const name = String(dealName || "").trim();
  const slotPrefix = name ? `${name} · ` : "";

  for (const comp of components) {
    const note = String(comp.note || "").split(" | ")[0].trim();
    if (name && note.startsWith(slotPrefix)) {
      choice.push(comp);
      continue;
    }
    if (
      comp.lineSource === "deal" ||
      (name && note === name)
    ) {
      fixed.push(comp);
      continue;
...
function inferDealQuantityFromComponents(deal, components = [], applied) {
  ...
  const dealName = String(applied?.dealName || deal?.name || "").trim();
  const { choice, fixed } = splitDealComponentsByRole(components, dealName);
  const qtyCandidates = [];

  const choiceSlots = (deal?.comboItems || []).filter(
    (ci) => getComboItemType(ci) === "choice",
  );
  if (choiceSlots.length > 0 && choice.length > 0) {
    const picksPerDeal = choiceSlots.reduce(
      (sum, ci) =>
        sum + Math.max(1, Number(ci.minSelect ?? ci.quantity ?? 1) || 1),
      0,
    );
    const choicePickQty = choice.reduce(
      (sum, c) => sum + (Number(c.qty ?? c.quantity) || 1),
      0,
    );
    if (picksPerDeal > 0) {
      qtyCandidates.push(Math.max(1, Math.round(choicePickQty / picksPerDeal)));
    }
  }

  const fixedSlots = (deal?.comboItems || []).filter(
    (ci) => getComboItemType(ci) === "fixed",
  );
  if (fixedSlots.length > 0 && fixed.length > 0) {
    const fixedPerDeal = fixedSlots.reduce(
      (sum, ci) => sum + (Number(ci.quantity) || 1),
      0,
    );
    const fixedQty = fixed.reduce(
      (sum, c) => sum + (Number(c.qty ?? c.quantity) || 1),
      0,
    );
    if (fixedPerDeal > 0) {
      qtyCandidates.push(Math.max(1, Math.round(fixedQty / fixedPerDeal)));
    }
  }

  if (qtyCandidates.length > 0) {
    return Math.min(...qtyCandidates);
  }

  const slotLabels = new Set();
  for (const comp of choice) {
  ...
  if (slotLabels.size > 0) {
    ...
    return Math.max(1, Math.round(choicePickQty / slotLabels.size));
  }
  ...
  if (comboPrice > 0) {
    ...
    return Math.max(1, Math.round(comboTotal / comboPrice));
  }
...
function inferComboAppliedFromOrderItems(items = [], availableDeals = []) {
  ...
  for (const dealName of dealNames) {
    ...
    const { choice } = splitDealComponentsByRole(components, dealName);
    const slotLabels = new Set();
    for (const comp of choice) {
      const note = String(comp.note || "").split(" | ")[0].trim();
    ...
    const picksPerDeal = Math.max(
      1,
      (dealDoc?.comboItems || []).filter((ci) => getComboItemType(ci) === "choice").length ||
        slotLabels.size ||
        choice.length,
    );
    const choicePickQty = choice.reduce(
    ...
    const dealQty = Math.max(1, Math.round(choicePickQty / picksPerDeal));
    const componentTotal = components.reduce(
      (sum, c) =>
        sum +
        (Number(c.lineTotal) ||
    ...
    // Signed: positive = discount, negative = surcharge when combo > components.
    const savings =
      comboPrice > 0
        ? Math.round((componentTotal - comboPrice * dealQty) * 100) / 100
        : 0;
...
export function rebuildPosCartFromOrder(order, menuItems = [], availableDeals = []) {
  ...
  for (const applied of comboApplied) {
    ...
    const appliedForName = comboApplied.filter(
      (d) => (d.dealName || "Deal") === dealName,
    );
    ...
    const components = items.filter((it) => isDealComponentLine(it, appliedForName));
    ...
    const dealQty =
      appliedForName.length > 1
        ...
        : inferDealQuantityFromComponents(
            dealDoc,
            components,
            appliedForName[0] || applied,
          );
    const discount = appliedForName.reduce(
