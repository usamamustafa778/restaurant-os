...
function getCookieDomain() {
  if (ROOT_DOMAIN) return `; domain=.${ROOT_DOMAIN}`;
  return "";
}
...
const _RESERVED_SEGMENTS = new Set([
...
function getTenantSlugFromLocation() {
  ...
  // Subdomain mode: extract from hostname (e.g. urbanspoon.sufieats.com)
  ...
  // Clean path mode: /<slug>/dashboard/... (e.g. /urbanspoon/dashboard/pos)
  const pathname = window.location.pathname;
  const cleanMatch = pathname.match(/^\/([^/]+)\//);
  if (cleanMatch && !_RESERVED_SEGMENTS.has(cleanMatch[1])) {
    return cleanMatch[1];
  }

  // Legacy /r/ path mode (e.g. /r/urbanspoon/dashboard)
  const rMatch = pathname.match(/^\/r\/([^/]+)\//);
  return rMatch ? rMatch[1] : null;
}
