...
// POS – create a new order via backend (branchId from context/localStorage is sent via x-branch-id; also in body for backend)
export async function createPosOrder({ items, orderType, paymentMethod, discountAmount, customerName, customerPhone, deliveryAddress, branchId, appliedDeals }) {
  const payload = { items, orderType, paymentMethod, discountAmount, customerName, customerPhone, deliveryAddress, appliedDeals };
