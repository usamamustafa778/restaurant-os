...
export async function getDaySessionOrders(sessionId) {
  // Avoid stale 304 cached responses when session totals change (OPEN day aggregates multiple sessions).
  return apiFetch(`/api/pos/day-session/${encodeURIComponent(sessionId)}/orders`, {
    cache: "no-store",
  });
}
