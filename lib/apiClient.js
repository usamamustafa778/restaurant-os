...
/** Tenant staff: custom roles for current restaurant (+ global). */
export async function getCustomRolesForRestaurant() {
  return apiFetch("/api/admin/custom-roles");
}

/** Super admin: permissions CRUD */
export async function getPermissions(params = {}) {
  const qs = new URLSearchParams();
  if (params.group) qs.set("group", params.group);
  if (params.active) qs.set("active", params.active);
  const q = qs.toString() ? `?${qs.toString()}` : "";
  return apiFetch(`/api/super-admin/roles-permissions/permissions${q}`);
}

export async function getPermissionsGrouped() {
  return apiFetch("/api/super-admin/roles-permissions/permissions/grouped");
}

export async function createPermission(body) {
  return apiFetch("/api/super-admin/roles-permissions/permissions", {
    method: "POST",
    body: JSON.stringify(body),
  });
}

export async function updatePermission(id, body) {
  return apiFetch(`/api/super-admin/roles-permissions/permissions/${encodeURIComponent(id)}`, {
    method: "PUT",
    body: JSON.stringify(body),
  });
}
...
export async function getCustomRoles(params = {}) {
  const qs = new URLSearchParams();
  if (params.restaurant) qs.set("restaurant", params.restaurant);
  if (params.active) qs.set("active", params.active);
  const q = qs.toString() ? `?${qs.toString()}` : "";
  return apiFetch(`/api/super-admin/roles-permissions/roles${q}`);
}

export async function getCustomRole(id) {
  return apiFetch(`/api/super-admin/roles-permissions/roles/${encodeURIComponent(id)}`);
}

export async function createCustomRole(body) {
  return apiFetch("/api/super-admin/roles-permissions/roles", {
    method: "POST",
    body: JSON.stringify(body),
  });
}

export async function updateCustomRole(id, body) {
  return apiFetch(`/api/super-admin/roles-permissions/roles/${encodeURIComponent(id)}`, {
    method: "PUT",
    body: JSON.stringify(body),
  });
...
export async function getRoleUsers(id) {
  return apiFetch(`/api/super-admin/roles-permissions/roles/${encodeURIComponent(id)}/users`);
}
