...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
function SectionSlider({ items, visibleDesktop, visibleMobile, renderItem, primaryColor, autoSlideDelay = 3000 }) {
  ...
  // Custom slow smooth scroll (800ms ease-in-out)
  ...
  // Use the smaller mobile width for SSR, then correct on client
  const gapPx = 20;
  ...
  const itemWidth = mounted ? (isMobile ? mobileWidth : desktopWidth) : mobileWidth;
          ...
          {Array.from({ length: totalDots }).map((_, dotIdx) => (
            <button
              ...
              style={{
                ...
                height: 8,
                backgroundColor: activeIndex === dotIdx ? (primaryColor || '#ef4444') : '#d1d5db',
              }}
            />
          ))}
        </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        {websiteSections.length > 0 ? (
          websiteSections.map((section, sIdx) => (
            section.items && section.items.length > 0 && (
              <section key={sIdx} id={`section-${sIdx}`} className={`py-16 ${sIdx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                <div className="max-w-7xl mx-auto px-4">
                  <div className="text-center mb-10">
                    <p className="text-sm font-bold uppercase tracking-widest mb-2" style={{ color: secondaryColor }}>
                      {section.subtitle || ''}
                    </p>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900">{section.title || `Section ${sIdx + 1}`}</h2>
                  </div>
                  ...
                  {sIdx === 0 && (
                    <SectionSlider
                      ...
                      primaryColor={primaryColor}
                      renderItem={(item) => (
                        <div className="group text-center cursor-pointer" onClick={() => openItem(item)}>
                          ...
                          <div className="mt-4">
                            ...
                            <p className="text-lg font-bold mt-1" style={{ color: primaryColor }}>PKR {item.price}</p>
                          </div>
                        </div>
                      )}
                    />
                  ...
                  {sIdx === 1 && (
                    <SectionSlider
                      ...
                      primaryColor={primaryColor}
                      renderItem={(item) => (
                        <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-lg  transition-all group cursor-pointer" onClick={() => openItem(item)}>
                          ...
                          <div className="p-3">
                            ...
                            <p className="text-base font-bold mt-1" style={{ color: primaryColor }}>PKR {item.price}</p>
                          </div>
                        </div>
                      )}
                    />
                  ...
                  {sIdx === 2 && (
                    <SectionSlider
                      ...
                      primaryColor={primaryColor}
                      renderItem={(item) => (
                        <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg group transition-shadow cursor-pointer" onClick={() => openItem(item)}>
                          <div className="relative h-48">
                            {item.imageUrl ? (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img src={item.imageUrl} alt={item.name} className="w-full group-hover:scale-105 transition-transform duration-300 h-full object-cover" />
                            ) : (
                              <div className="w-full h-full bg-gradient-to-br from-gray-200 to-gray-300" />
                            )}
                          </div>
                          <div className="p-4">
                            ...
                            <p className="text-sm text-gray-600 mb-4 max-w-[250px] line-clamp-2 ">{item.description}</p>
