...
function getSalesReportQuery(preset, sessions, cutoffHour = 4) {
  ...
  if (safePreset === "today" && sessions && sessions.length > 0) {
    ...
    if (open?.id) {
      ...
      const openDate = open.startAt
        ? new Date(open.startAt).toDateString()
        : null;
...
export default function HistoryPage() {
  ...
  const [showSessionsDateDropdown, setShowSessionsDateDropdown] =
    useState(false);
  ...
  function openFullDayCombinedReport(day) {
    ...
    const rep = sessions.find((s) => s.status === "OPEN") || sessions[0];
  ...
  function handleExportCSV() {
    ...
    if (activeTab === "overview") {
      ...
      rows.push(
        ...
        ...(discountReport.byDay || []).map((r) => [r.day, r.count, r.total]),
      );
          ...
          <>
            <button
              ...
              onClick={(e) => {
                ...
                setPaymentDropdownId(
                  paymentDropdownId === rowId ? null : rowId,
                );
              }}
              ...
              <table className="text-xs min-w-[2000px]">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-neutral-800/60">
                  {paged.map((o, i) => (
                    ...
                    >
                      <td
                        className={`${TD_CLS} text-center text-gray-400 dark:text-neutral-600 select-none`}
                      >
                      ...
                      <td className={TD_CLS}>
                        {(() => {
                          ...
                          return (
                            <span
                              className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${STYLE_MAP[src] || "bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-neutral-400"}`}
                            >
                              {LABEL_MAP[src] || src}
                            </span>
                          );
  ...
  function renderSessions() {
    ...
    const filteredSessionOrders = (() => {
      const q = String(sessionOrderSearch || "")
        .trim()
        .toLowerCase();
                  ...
                  return (
                    <div className="divide-y divide-gray-100 dark:divide-neutral-800/70">
                      {grouped.map((day) => {
                        ...
                        const weekday = d.toLocaleDateString("en-PK", {
                          weekday: "long",
                        });
                              ...
                              <div className="flex items-center gap-3 px-4 py-2.5">
                                ...
                                <div className="flex items-center gap-2.5 flex-shrink-0">
                                  ...
                                  <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700">
                                    ...
                                    <span className="text-[12px] font-semibold text-gray-700 dark:text-neutral-300">
                                  ...
                                  <div className="px-2.5 py-1 rounded-lg bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/25">
                                    <span className="text-[13px] font-extrabold text-primary">
                                      {fmtRs(day.totalSales)}
                                    </span>
                                  </div>

                                  <ChevronRight
                                    className={`w-3.5 h-3.5 flex-shrink-0 transition-all ${isExpanded ? "opacity-0" : "text-gray-300 dark:text-neutral-600 group-hover:text-primary"}`}
                                  />
                                </div>
                              </div>
                            </button>
                              ...
                              <div className="border-t border-gray-100 dark:border-neutral-800 bg-gray-50/40 dark:bg-neutral-900/30">
                                {day.sessions.map((s, idx) => (
                                  ...
                                  >
                                    ...
                                    <div
                                      className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold border ${
                                        s.status === "OPEN"
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-2">
                                        <span
                                          className={`text-[12px] font-semibold ${s.status === "OPEN" ? "text-emerald-600 dark:text-emerald-400" : "text-gray-700 dark:text-neutral-300"}`}
                                        >
                                          {s.status === "OPEN"
                                            ? "Current session"
                                            : `Session ${idx + 1}`}
                                        ...
                                        <span className="text-[11px] text-gray-400 dark:text-neutral-500">
                                          ...
                                          {s.endAt
                                            ? ` → ${new Date(s.endAt).toLocaleTimeString("en-PK", { hour: "2-digit", minute: "2-digit", hour12: true })}`
                                            : " → Ongoing"}
                                    ...
                                    <div className="flex items-center gap-3 flex-shrink-0">
                                      ...
                                      <span className="text-[12px] font-bold text-gray-800 dark:text-neutral-200">
                                        {fmtRs(s.totalSales)}
                                      </span>
                ...
                <div>
                  ...
                  {sessionDetailScope === "day" ? (
                    <p className="text-[11px] text-gray-500 dark:text-neutral-400 mt-0.5">
                      All sessions on this calendar day combined — same totals
                      as the day row
                    </p>
                  ) : (
                          ...
                          {sessionUnpaid.totalCount > 0 ? (
                            <div className="mt-1 space-y-0.5 text-[10px] text-amber-600 dark:text-amber-400">
                              ...
                              <p className="text-[9px] leading-snug opacity-95">
                                ...
                                ({sessionUnpaid.pipelineCount}) · Delivered,
                                payment pending:{" "}
                                {fmtRs(sessionUnpaid.deliveredAmt)} (
                          ...
                          <div className="mt-1.5 grid grid-cols-1 sm:grid-cols-3 gap-1.5">
                            ...
                            <div className="rounded-md bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1.5">
                              ...
                              <p className="text-[12px] font-bold text-emerald-700 dark:text-emerald-400">
                                {fmtMoney(countedCashTotal)}
                              </p>
                            </div>

                            <div
                              className={`rounded-md px-2 py-1.5 ${cashDiff === 0 ? "bg-gray-50 dark:bg-neutral-800/60" : cashDiff > 0 ? "bg-amber-50 dark:bg-amber-500/10" : "bg-rose-50 dark:bg-rose-500/10"}`}
                            >
                              <p className="text-[9px] uppercase tracking-wide text-gray-400 dark:text-neutral-500">
                                Difference
                              </p>
                              ...
                              <p
                                className={`text-[12px] font-bold ${cashDiff === 0 ? "text-gray-900 dark:text-white" : cashDiff > 0 ? "text-amber-700 dark:text-amber-400" : "text-rose-700 dark:text-rose-400"}`}
                              >
                                {cashDiff > 0 ? "+" : ""}
                                  ...
                                  >
                                    ...
                                    <td className="py-2.5 px-3 text-[12px] whitespace-nowrap">
                                      {o.source === "WEBSITE" ? (
                                        ...
                                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-violet-100 dark:bg-violet-500/15 text-violet-700 dark:text-violet-400 font-bold">
                                          POS
                                        </span>
                                      )}
                                    </td>
          ...
          <div className="bg-white dark:bg-neutral-950 border border-gray-200 dark:border-neutral-800 rounded-2xl shadow-sm p-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <nav className="flex items-stretch sm:items-center gap-1" aria-label="Report views">
              {TABS.map((tab) => {
                ...
                if (
                  tab.id === "discounts" &&
                  discountReport?.orderCount != null
                )
            ...
            {/* Controls: Date, Export, Help — hidden on sessions tab */}
            {activeTab !== "sessions" && (
                ...
                <div className="relative flex-1 sm:flex-none">
                  ...
                  {showDateDropdown && (
                    <>
                      ...
                      />
                      <div className="absolute left-0 sm:right-0 sm:left-auto top-full mt-1.5 z-50 w-72 bg-white dark:bg-neutral-950 border-2 border-gray-200 dark:border-neutral-700 rounded-2xl shadow-xl overflow-hidden">
                        <div className="p-2 space-y-0.5">
                          {PRESETS.filter((p) => p.id !== "custom").map((p) => (
                            <button
                              ...
                              className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-all ${preset === p.id ? "bg-gradient-to-r from-primary to-secondary text-white" : "text-gray-700 dark:text-neutral-300 hover:bg-gray-100 dark:hover:bg-neutral-800"}`}
                            >
                              {p.label}
                            </button>
                          ))}
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setOrdersExportColumns(
                                      ORDER_EXPORT_COLUMN_OPTIONS.map(
                                        (c) => c.key,
                                      ),
                                    )
                                  }
            ...
            {activeTab === "sessions" && (
              <div className="flex items-center gap-1.5 sm:flex-shrink-0 flex-wrap">
                  ...
                  return (
                    <div className="relative flex-1 sm:flex-none">
                      <button
                        ...
                        className={`w-full inline-flex items-center gap-1.5 h-8 px-3 rounded-xl border text-xs font-semibold transition-all ${isFiltered ? "border-primary/40 bg-primary/5 dark:bg-primary/10 text-primary" : "border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-900 text-gray-700 dark:text-neutral-300 hover:border-primary/40"}`}
                      >
                        <Calendar
                          className={`w-3.5 h-3.5 flex-shrink-0 ${isFiltered ? "text-primary" : "text-gray-400"}`}
                        />
                        <span className="flex-1 text-left truncate">
                          {presetLabel}
                        </span>
                        {isFiltered ? (
                          ...
                          <ChevronDown
                            className={`w-3 h-3 transition-transform ${showSessionsDateDropdown ? "rotate-180" : ""}`}
                          />
                        )}
                      </button>
                      {showSessionsDateDropdown && (
                        <>
                          <div
                            className="fixed inset-0 z-40"
                            onClick={() => setShowSessionsDateDropdown(false)}
                          />
                            ...
                            <div className="p-2 space-y-0.5">
                              {SESSION_DATE_PRESETS.filter(
                                ...
                                <button
                                  ...
                                  type="button"
                                  onClick={() => {
                                    const dates = getSessionPresetDates(p.id);
                                    setSessionsDatePreset(p.id);
                                    setSessionsDateFrom(dates.from);
                                    setSessionsDateTo(dates.to);
                                    setSessionsPage(0);
                                    loadSessions(0, dates.from, dates.to);
                                    setShowSessionsDateDropdown(false);
                                  }}
                                  className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-all ${sessionsDatePreset === p.id ? "bg-gradient-to-r from-primary to-secondary text-white" : "text-gray-700 dark:text-neutral-300 hover:bg-gray-100 dark:hover:bg-neutral-800"}`}
                                >
                                  {p.label}
                                </button>
                              ))}
                            ...
                            <div className="border-t border-gray-100 dark:border-neutral-800 p-3">
                              ...
                              <div className="grid grid-cols-2 gap-2 mb-2">
                                <input
                                  ...
                                  className="h-8 px-2.5 rounded-lg bg-gray-50 dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700 text-xs text-gray-900 dark:text-white outline-none focus:border-primary"
                                />
                                <input
                                  type="date"
                              ...
                              <button
                                ...
                                className="w-full h-8 rounded-lg bg-gradient-to-r from-primary to-secondary text-white text-xs font-semibold disabled:opacity-50"
                              >
                                Apply
                              </button>
                            </div>
                ...
                <div className="flex items-center bg-gray-100 dark:bg-neutral-800 rounded-xl p-0.5 gap-0.5">
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className={`flex items-center gap-1.5 h-7 px-3 rounded-lg text-[11px] font-semibold transition-all ${sessionsViewMode === "daily" ? "bg-white dark:bg-neutral-700 text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-neutral-400"}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 h-7 px-3 rounded-lg text-[11px] font-semibold transition-all ${sessionsViewMode === "sessions" ? "bg-white dark:bg-neutral-700 text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-neutral-400"}`}
                  >
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-900 flex items-center justify-center text-gray-400 hover:text-primary transition-all flex-shrink-0"
                  title="Refresh"
                >
                  <RefreshCw
          ...
          >
            <div className="px-6 py-5 border-b border-gray-100 dark:border-neutral-800 flex items-start justify-between bg-gradient-to-b from-gray-50/80 to-white dark:from-neutral-900/60 dark:to-neutral-950">
              <div className="min-w-0">
                ...
                <h3 className="text-2xl font-black text-gray-900 dark:text-white leading-tight truncate mt-0.5">
                  ...
                  {selectedOrderDetail.orderNumber ||
                    selectedOrderDetail.id?.slice(-4)}
                </h3>
              ...
              <div className="flex items-center gap-2 ml-3">
                <span
                  className={`inline-flex items-center px-3 py-1.5 rounded-full text-[11px] font-semibold ${STATUS_COLORS[selectedOrderDetail.status] || "bg-gray-100 text-gray-500"}`}
                >
                  {STATUS_LABELS[selectedOrderDetail.status] ||
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(88vh-84px)] space-y-5">
                ...
                <div className="rounded-2xl border border-gray-200 dark:border-neutral-800 p-4 bg-white dark:bg-neutral-950 shadow-sm">
                  ...
                  <div className="space-y-2.5 text-sm">
                    <div className="flex justify-between gap-3">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white">
                        {(selectedOrderDetail.orderType || "").replace(
                          /_/g,
                          " ",
                        ) || "—"}
                      </span>
                    ...
                    <div className="flex justify-between gap-3">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white">
                        {selectedOrderDetail.isPaid
                          ? selectedOrderDetail.paymentMethod || "Paid"
                          : "Unpaid"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2.5 text-sm">
                    ...
                    <div className="flex justify-between gap-3">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white text-right">
                        {selectedOrderDetail.assignedRiderName ||
                        selectedOrderDetail.riderName
                          ? `Rider: ${selectedOrderDetail.assignedRiderName || selectedOrderDetail.riderName}`
                          : selectedOrderDetail.waiterName
                            ? `Waiter: ${selectedOrderDetail.waiterName}`
                            : selectedOrderDetail.orderTakerName
                              ? `Taker: ${selectedOrderDetail.orderTakerName}`
                              : selectedOrderDetail.createdBy?.name
                                ? `Taker: ${selectedOrderDetail.createdBy.name}`
                                : "—"}
                      </span>
                    </div>
              ...
              {String(selectedOrderDetail.paymentMethod || "").toUpperCase() ===
                "SPLIT" && (
                <div className="rounded-2xl border border-amber-200 dark:border-amber-500/30 bg-gradient-to-r from-amber-50/80 to-orange-50/40 dark:from-amber-500/10 dark:to-amber-500/5 p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-sm">
                    <div className="rounded-xl bg-white/90 dark:bg-neutral-900/70 px-3.5 py-2.5 border border-amber-100 dark:border-amber-500/20">
                      ...
                      <p className="font-extrabold text-amber-900 dark:text-amber-100 text-base">
                        {fmtRs(
                          Number(selectedOrderDetail.splitCashAmount) || 0,
                        )}
                      </p>
                    </div>
                    ...
                    <div className="rounded-xl bg-white/90 dark:bg-neutral-900/70 px-3.5 py-2.5 border border-amber-100 dark:border-amber-500/20">
                      ...
                      <p className="font-extrabold text-amber-900 dark:text-amber-100 text-base">
                        {fmtRs(
                          Number(selectedOrderDetail.splitCardAmount) || 0,
                        )}
                      </p>
                    </div>
                    ...
                    <div className="rounded-xl bg-white/90 dark:bg-neutral-900/70 px-3.5 py-2.5 border border-amber-100 dark:border-amber-500/20">
                      ...
                      <p className="font-extrabold text-amber-900 dark:text-amber-100 text-base">
                        {fmtRs(
                          Number(selectedOrderDetail.splitOnlineAmount) || 0,
                        )}
                      </p>
                      <p className="text-[10px] text-amber-700/80 dark:text-amber-300/80 truncate mt-0.5">
                        {selectedOrderDetail.splitOnlineProvider || "—"}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-neutral-800">
                      {(selectedOrderDetail.items || []).map((it, idx) => (
                        <tr
                          key={idx}
                          className="hover:bg-gray-50/70 dark:hover:bg-neutral-900/40"
                        >
                          <td className="px-4 py-2.5 font-medium text-gray-900 dark:text-white">
                            {it.name || "Item"}
                          </td>

                          <td className="px-2 py-2.5 text-center text-gray-600 dark:text-neutral-400">
                            {it.qty || it.quantity || 1}
                          </td>

                          <td className="px-4 py-2.5 text-right font-semibold text-gray-900 dark:text-white">
                            {fmtRs(Number(it.lineTotal ?? 0))}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="border-t border-gray-100 dark:border-neutral-800 pt-2 mt-2 flex justify-between gap-3 items-baseline">
                    ...
                    <span className="text-lg font-black text-primary tabular-nums">
                      {fmtRs(
                        Number(
                          selectedOrderDetail.grandTotal ??
